...
How we know its overfit or underfit? 
...
tree_depth = 9 and conditional_min_criterion = 0.999954
...
```
Why the 4 horizontal lines?  fitting the model to the training set only and then uses the model to do predictions on the test set

we didnt see these parallel lines (horizontal lines) in the elasticnet model. 
...
```
only mean_srad.wm2_jul (solar radiation) and mean_vp.pa_jun (vapor pressure) entered the model
...
mean_vp.pa_May (vapor pressure), mean_srad.wm2_Aug (solar radiation) and mean_dayl.s_Sep (daylength) entered the model
